...
- [x] 1.3 Add `.gitignore` (node_modules, `.expo`, `dist`, `.turbo`, `coverage`, `expo-env.d.ts`, `ios/`, `android/`, `.env*.local`, `*.jks`, `*.p8`, `*.p12`, `*.key`, `*.mobileprovision`) and `.editorconfig`. Verify that `git status` shows no `node_modules/`
